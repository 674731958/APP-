  ...
  <div class="col-md-12 col-sm-12 col-xs-12">
    <div class="x_panel">
	  ...
	  <!-- <div class="item form-group">
           ...
           <div class="clearfix"></div>
        <form class="form-horizontal form-label-left" action="../dev/insertappinfo.do" method="post" enctype="multipart/form-data">
          <div class="item form-group">
...
$(document).ready(function(){
	...
	$("#categoryLevel1").change(function(){
		...
		if(categoryLevel1!=null&&categoryLevel1!=''){
			$.getJSON("categorylevellist.do",{"queryCategoryLevel1":categoryLevel1},function(data){	
				$("#categoryLevel2").html("");	
				var options="<option value='0'>--请选择--</option>"
				$(data).each(function(){
					options+="<option value="+this.id+">"+this.categoryName+"</option>";
	...
	$("#categoryLevel2").change(function(){
		...
		if(categoryLevel2!=null&&categoryLevel2!=''){
			$.getJSON("categorylevellist2.do",{"queryCategoryLevel2":categoryLevel2},function(data){	
				$("#categoryLevel3").html("");	
				var options="<option value='0'>--请选择--</option>"
				$(data).each(function(){
					options+="<option value="+this.id+">"+this.categoryName+"</option>";
	...
	$("#APKName").blur(function(){
		...
		$.getJSON("checkASPName.do",{"APKName":apkName},function(data){
			if(data==0){
				alert("该APK名可以使用");
			}else{
				alert("该APK名已存在，请重新输入");
			}
		})
	})
})
